...
function InputSearch(props: IProps) {
    const { search, setSearch, list, setList, getColorInfo } = props

    const getColorValueFromInput = (
        event: React.KeyboardEvent<HTMLDivElement>
    ) => {
        const enteredText = (event.target as HTMLInputElement).value.trim()
        if (event.key === 'Enter') {
            const { sortedColorList, hex }: ISearchInfo = searchColour(
                enteredText,
                list.list
            )
            ...
            <Col lg="5" md="5" sm="12">
                ...
                <div className={style.InputSearch__inputContainer}>
                    <Input
                        ...
                        ) => searchByColour(event.target.value)}
                    />
